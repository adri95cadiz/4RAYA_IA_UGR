...
void Environment::AcceptAction(Environment::ActionType action)
{
    ...
    switch (action){
		case Environment::PUT1:
                                if (n_jugada%4==0 and !explosion[jugador_activo_]){
...
void Environment::AplicarExplosion(int i, int j){
    ...
    for (int x=-1; x<2; x++)
        if (i+x>=0 and i+x<7)
            if (maze_[i+x][j]==adversario+3)
                explosion[adversario]=false;
    // Segundo la fila
    for (int x=-1; x<2; x++)
        if (j+x>=0 and j+x<7)
            if (maze_[i][j+x]==adversario+3)
                explosion[adversario]=false;

    // Eliminar la cruz y desplazar las casillas a los huecos
    // Empiezo por los de la columna

    // Elimino la casilla de la izquierda y aplica la gravedad
    if (j-1>=0 and maze_[i][j-1]!=0){
        maze_[i][j-1]=0;
        casillas_libres_++;
        int k=i;
        while (k<6){
            maze_[k][j-1]=maze_[k+1][j-1];
            k++;
        }
        maze_[6][j-1]=0;
        tope_[j-1]--;
    ...
    // Elimino la casilla de la derecha y aplica la gravedad
    if (j+1<7 and maze_[i][j+1]!=0){
        maze_[i][j+1]=0;
        casillas_libres_++;
        int k=i;
        while (k<6){
    ...
    //Elimino la casilla de abajo y aplico gravedad
    int inferior=i;
    if (i-1>=0){
        maze_[i-1][j]=0;
        casillas_libres_++;
